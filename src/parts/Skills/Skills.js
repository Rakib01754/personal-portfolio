import React from 'react';

const Skills = () => {
    return (
        <section class="bg-gray-200">
            <div class="container px-5 py-10 mx-auto">
                <div class="flex flex-col text-center w-full mb-20">
                    <h1 class="sm:text-5xl text-3xl font-medium title-font mb-4 text-gray-900">My Skills</h1>
                </div>
                <div class="flex flex-wrap -m-2">
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100  object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/721/721671.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Javascript</h2>
                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/1126/1126012.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">React.js</h2>
                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/5968/5968322.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Node.js</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="data:image/png;base64," />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Express.js</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9ujkq8h_-8_v77Nfc4SHP8lDu-4D7JK9DDA&usqp=CAU" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">MongoDB</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu8Ipu-vQqFgklgrYRhAZk7YW3Gk7a5d5YFX9rpxxRLTichiVuepft&usqp=CAE&s" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Firebase</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTji39I4JAESu9ZodDFkz0u_APBrWjIpIBVdw&usqp=CAU" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Tailwind Css</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/5968/5968672.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Bootstrap</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3  w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/732/732212.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">HTML5</h2>

                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3 w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/732/732190.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">CSS3</h2>
                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3 w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://cdn-icons-png.flaticon.com/128/25/25657.png" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Github</h2>
                            </div>
                        </div>
                    </div>
                    <div class="p-2 lg:w-1/4 md:w-1/3 w-1/2">
                        <div class="h-full flex items-center border-white border p-4 rounded-lg shadow-lg">
                            <img alt="team" class="w-16 h-16 bg-gray-100 object-cover object-center flex-shrink-0 rounded-full mr-4" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZgA12maOTY4A_4kASmwSFgAlTg_MCzk8r6w&usqp=CAU" />
                            <div class="flex-grow">
                                <h2 class="text-gray-900 title-font font-bold text-xl">Netlify</h2>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    );
};

export default Skills;